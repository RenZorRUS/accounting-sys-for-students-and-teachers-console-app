...
def edit_rows_in(db_cursor) -> None:
    """Edit rows in selected table"""

    screen_clear()

    print(
        """In which table do you want to edit data?
    ...
    while True:
        try:
            ...
            if option in (1, 2, 3, 4, 5):
                ...
                obj = options[option]()
                while True:
                    obj.edit_row(db_cursor)
                    option = input('Continue editing data? [Y, y | N, n]: ')
                    if option in ('Y', 'y'):
                        continue
                    elif option in ('N', 'n'):
                        break
